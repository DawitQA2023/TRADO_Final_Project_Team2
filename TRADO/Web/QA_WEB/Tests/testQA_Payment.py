import allure
import pytest
from TRADO.Web.QA_WEB.Pages.paymentQA_page import Payment_Page
...
class TestQA_Payment_Chrome1(Pre_Requirement_Login_Chrome):
    ...
    def test_Valid_Product_check_out_by_using_all_valid_fields(self):
        ...
        payments.Enter_all_Invoice_detail_filed("user-3p7is6i1lediwnhm", "1", "team2@gmail.com", "Beersheba, Israel",
                                                "Addis Ababa, Ethiopia", "105", "2", "2")
        payments.Enter_all_Shipping_Address_filed('Beersheba, Israel', 'Addis Ababa, Ethiopia', '105', '2', '2',
                                                  'dawit', 'david', 'sami', '3322114455')
        payments.Click_Complete_Purchase_button()
        Utils(driver).assertion(driver.title, 'אקדיה לרכישה אונליין – Trado')
    ...
    @pytest.mark.sanity
    @allure.description('Trying to purchase without business name ')
    @allure.severity(allure.severity_level.CRITICAL)
    def test_check_out_by_without_business_name_fields(self):
        driver = self.driver
        payments = Payment_Page(driver)
        payments.Valid_open_payment_page()
        ...
        payments.Enter_all_Shipping_Address_filed('Beersheva', 'nurit', '1', '1', '2', 'TEAM2', 'TEAM', 'TWO',
                                                  '2444666666')
        payments.Click_Complete_Purchase_button()
        Utils(driver).assertion(payments.driver.title, 'אקדיה לרכישה אונליין – Trado')
...
@pytest.mark.usefixtures('Valid_Login_With_login_Code')
class TestQA_Payment_Firefox(Pre_Requirement_Login_Firefox):
    @pytest.mark.sanity
    @allure.description('Successfully fill all the detail for purchase a product')
    @allure.severity(allure.severity_level.CRITICAL)
    def test_Valid_Product_check_out_by_using_all_valid_fields(self):
...
# ------------------------------------------E2E Product Add to Check out -------------------------------------------------------


@pytest.mark.usefixtures('Valid_Login_With_login_Code')
class TestQA_Payment_Chrome2_(Pre_Requirement_Login_Chrome):
    @pytest.mark.sanity
    @allure.description('Successfully fill all the detail for purchase a product')
    @allure.severity(allure.severity_level.CRITICAL)
    def test_Valid_Product_check_out_E2E(self):
        driver = self.driver
        payments = Payment_Page(driver)
        payments.Valid_open_payment_page_with_all_product()  # add 3 products
        payments.Click_Payment_BUTTON_()
        payments.Enter_all_Invoice_detail_filed("user-3p7is6i1lediwnhm", "143677", "team2@gmail.com",
                                                "Beersheba, Israel",
                                                "Addis Ababa, Ethiopia", "105", "2", "2")
        payments.Enter_all_Shipping_Address_filed('Beersheba, Israel', 'Addis Ababa, Ethiopia', '105', '2', '2',
                                                  'dawit', 'david', 'sami', '3322114455')
        payments.Click_Complete_Purchase_button()
        payments.Click_Bank_Transfer_Icon_button()
        payments.Enter_BankTransfer_form("2", "0123456789")
        payments.Click_ConfirmTransfer_button()
        text = payments.Assert_text(Locators_Payment.MYWALLET_TEXT)
        Utils(driver).assertion(text, 'הארנק שלי')
